...
def calculate_upper_arm_angle_from_trunk(shoulder, elbow, trunk_vector):
    """
    Calculate upper arm angle relative to trunk axis (ergonomically correct).
    
    Args:
        shoulder: (x, y) - shoulder position
        elbow: (x, y) - elbow position
        trunk_vector: (x, y) - normalized trunk direction vector
    
    Returns:
        angle in degrees (deviation from trunk axis)
    """
    # Upper arm vector: shoulder → elbow
    upper_arm_vector = np.array([elbow[0] - shoulder[0], elbow[1] - shoulder[1]])
    
    # Calculate magnitudes
    upper_arm_magnitude = np.linalg.norm(upper_arm_vector)
    trunk_magnitude = np.linalg.norm(trunk_vector)
    
    if upper_arm_magnitude == 0 or trunk_magnitude == 0:
        return 0
    
    # Normalize and compute dot product
    upper_arm_normalized = upper_arm_vector / upper_arm_magnitude
    trunk_normalized = trunk_vector / trunk_magnitude
    dot_product = np.dot(upper_arm_normalized, trunk_normalized)
...
def score_upper_arm(angle, is_abducted=False):
    """
    Official RULA scoring for upper arm angle (relative to trunk).
    
    Args:
        angle: Upper arm angle in degrees (deviation from trunk axis)
...
def score_wrist(angle):
    ...
    Note: In 2D pose estimation, true wrist flexion/extension is difficult.
    We approximate using forearm-to-horizontal deviation.
    
    Args:
        angle: Wrist deviation angle in degrees
...
def score_neck(neck_internal_angle):
    """
    Official RULA scoring for neck angle.
    Converts internal angle to neck flexion.
    
    Args:
        neck_internal_angle: Internal angle at shoulder center (nose-shoulder-hip)
    
    Returns:
        Score (1-4)
    """
    # Convert to neck flexion (forward bend)
    neck_flexion = 180 - neck_internal_angle
    ...
    if neck_flexion <= 10:
        ...
        return 3
    else:  # Extension (negative flexion)
        return 4
...
def score_legs(hips_detected=True, symmetric=True):
    """
    Official RULA scoring for legs (seated posture).
    
    Args:
        hips_detected: Whether hip keypoints are detected
        symmetric: Whether hips appear symmetrical
    
    Returns:
        Score (1-2)
    """
    # For stable seated posture with feet supported
    if hips_detected and symmetric:
        return 1
    else:
        return 2
...
def calculate_official_rula(upper_arm_angle, lower_arm_angle, wrist_angle, 
    ...
    legs_score = score_legs(hips_detected, symmetric=True)
    ...
    # Convert neck angle to flexion for display
    neck_flexion = 180 - neck_angle
...
def draw_elbow_angles(frame, keypoints, conf_threshold=0.3, debug=True):
    ...
    for person_idx, person_kps in enumerate(keypoints):
        ...
        # ============================================================
        # UPPER ARM ANGLE (RULA Group A) - Relative to Trunk
        # ============================================================
        ...
        if (right_sh[2] > upper_arm_conf_threshold and 
            right_el_kp[2] > upper_arm_conf_threshold and 
            trunk_vector is not None):
            
            # Calculate upper arm angle RELATIVE TO TRUNK (ergonomically correct)
            upper_arm_angle = calculate_upper_arm_angle_from_trunk(
                (right_sh[0], right_sh[1]),
        ...
        # ============================================================
        # WRIST ANGLE (SIMPLIFIED)
        # ============================================================
        # Calculate angle between forearm and horizontal reference
        ...
        if right_el_kp[2] > wrist_conf_threshold and right_wr_kp[2] > wrist_conf_threshold:
            # Create horizontal reference point (100 pixels to the right of wrist)
            horizontal_ref = (right_wr_kp[0] + 100, right_wr_kp[1])
            
            # Calculate wrist angle
            wrist_angle = calculate_angle(
                (right_el_kp[0], right_el_kp[1]),  # elbow
                (right_wr_kp[0], right_wr_kp[1]),  # wrist (vertex)
                horizontal_ref                      # horizontal reference
            )
            
            # Draw forearm line (green) - already drawn above for elbow, so make it thicker here
            cv2.line(frame,
                    (int(right_el_kp[0]), int(right_el_kp[1])),
            ...
            cv2.line(frame,
                    (int(right_wr_kp[0]), int(right_wr_kp[1])),
                    (int(horizontal_ref[0]), int(horizontal_ref[1])),
                    (0, 255, 255), 1)  # Yellow reference line
        ...
        if (nose_kp[2] > conf_threshold and shoulder_center is not None and hip_center is not None):
            ...
            # Calculate neck angle (nose → shoulder_center → hip_center)
            ...
            # ============================================================
            # Check if all required angles are available
            required_angles = ['upper_arm', 'lower_arm', 'wrist', 'neck', 'trunk']
            if all(angle_key in rula_angles for angle_key in required_angles):
                # Check if hips are detected
                hips_available = (left_hp_kp[2] > torso_threshold and 
                                 right_hp_kp[2] > torso_threshold)
